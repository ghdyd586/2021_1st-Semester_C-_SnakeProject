...
//Snake moving --------------------------------------------------------------------------------------
    ...
    // directon decide
      ...
      snake_move(snake, gate, &snake_direction, &gate_crossing, &cross_cnt);
      color_apply(selmap, win1);
      ...
      if(cnt%40==0&&!gate_crossing){ //if snake is not crossing the gate, generate gate per 10sec
        make_gate(selmap, gate);
        cnt=0; //initialize to keep cnt in range
      }
      color_apply(selmap, win1);
      refresh();
