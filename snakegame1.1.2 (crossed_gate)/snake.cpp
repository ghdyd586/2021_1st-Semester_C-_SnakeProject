...
// moving process--------------------------------------------------------------------------------
void snake_move(vector<snakebody>& snake, vector<snakebody>& gate, int* snake_direction, bool* gate_crossing, int* cross_cnt){
  ...
  if(*gate_crossing){ //if snake is crossing the gate
    if(*cross_cnt==snake.size()){
      *cross_cnt=0;
      make_gate(selmap, gate); //new gate generates when snake crossed the gate
      *gate_crossing=false;
    }
     *cross_cnt+=1;
  }
